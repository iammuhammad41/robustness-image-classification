...
###############################################################################
# adding distillation util and kd loss
###############################################################################
...
# parser.add_argument('--teacher-model', type=str, default='./results/resnet50_lr_0.001/model_best.pth.tar', help='the temperature of teacher model for knowledge distillation')
parser.add_argument('--teacher-model', type=str, default='./results/resnet44_lr_0.05/model_best.pth.tar', help='the temperature of teacher model for knowledge distillation')
parser.add_argument('--teacher-temperature', type=float, default=2 , help='the temperature of teacher model for knowledge distillation')
parser.add_argument('--kd-weight', type=float, default=1 , help='how much weight of teacher loss should be in the total loss')
best_prec1 = 0
...
def main():
    ...
    ###############################################################################
    t_model = resnet50(pretrained=False)
    t_fc_in = t_model.fc.in_features
    t_model.fc = nn.Linear(t_fc_in, 200)
    t_model = t_model.cuda()
    t_checkpoint = torch.load(args.teacher_model)
    t_model.load_state_dict(t_checkpoint['state_dict'])
    print(t_model)
    t_model.eval()
    for param in t_model.parameters():
        param.requires_grad = False
    # t_forward_hook_manager = ForwardHookManager('cuda:0')
    # t_forward_hook_manager.add_hook(t_model, 'layer1', requires_input=True, requires_output=True)
    # t_forward_hook_manager.add_hook(t_model, 'layer2', requires_input=True, requires_output=True)
    # t_forward_hook_manager.add_hook(t_model, 'layer3', requires_input=True, requires_output=True)
    # t_forward_hook_manager.add_hook(t_model, 'layer4', requires_input=True, requires_output=True)
    # t_forward_hook_manager.add_hook(t_model, 'fc', requires_input=True, requires_output=True)
    ###############################################################################


    s_model = modeltype(num_classes=args.num_classes)
    # s_model = modeltype()
    # s_forward_hook_manager = ForwardHookManager('cuda:0')
    # s_forward_hook_manager.add_hook(t_model, 'layer1', requires_input=True, requires_output=True)
    # s_forward_hook_manager.add_hook(t_model, 'layer2', requires_input=True, requires_output=True)
    # s_forward_hook_manager.add_hook(t_model, 'layer3', requires_input=True, requires_output=True)
    # s_forward_hook_manager.add_hook(t_model, 'layer4', requires_input=True, requires_output=True)
    # s_forward_hook_manager.add_hook(t_model, 'fc', requires_input=True, requires_output=True)
    print(s_model)
    ...
    # kd_loss
    ###############################################################################
    kd_criterion = SoftTarget(args.teacher_temperature)
    ###############################################################################
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        s_train_obj, t_train_obj, top1, top5 = train(train_loader, s_model, t_model, classification_criterion, kd_criterion, optimizer, epoch)
        # evaluate on validation set
        s_val_obj, t_val_obj, prec1, prec5 = validate(val_loader, s_model, t_model, classification_criterion, kd_criterion)
        # update stats
...
def train(train_loader, s_model, t_model, classification_criterion, kd_criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        kd_loss = kd_criterion(s_output, t_output.detach())
        # loss = (1-args.kd_weight) * classification_loss + args.kd_weight * kd_loss
        loss = classification_loss + (args.kd_weight * kd_loss)

        # measure accuracy and record loss
        prec1, prec5 = accuracy(s_output, target, topk=(1, 5))
        classification_losses.update(loss.item(), input.size(0))
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                   ...
                   data_time=data_time, cls_loss=classification_losses, kd_loss = kd_losses, top1=top1, top5=top5))
    return classification_losses.avg, kd_losses.avg, top1.avg, top5.avg


def validate(val_loader, s_model, t_model, classification_criterion, kd_criterion):
    batch_time = AverageMeter()
    classification_losses = AverageMeter()
    kd_losses = AverageMeter()
    top1 = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            t_loss = kd_criterion(s_output, t_output.detach())

            # measure accuracy and record loss
            prec1, prec5 = accuracy(s_output, target, topk=(1, 5))
            classification_losses.update(s_loss.item(), input.size(0))
            kd_losses.update(t_loss.item(), input.size(0))
            ...
            if i % args.print_freq == 0:
                print('Test: [{0}/{1}]\t'
                      ...
                      'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                      'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                       i, len(val_loader), batch_time=batch_time, cl_loss=classification_losses, kd_loss = kd_losses,
                       top1=top1, top5=top5))
